...
export const sendGroupInvitationEmail = async ({
  ...
  const dashboardLink = `${FRONTEND_URL || "http://localhost:5173"
    }/family-sharing`;
...
export const sendPaymentReminderEmail = async ({
  ...
  try {
    ...
    const { data, error } = await resend.emails.send({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 10px;">
          <h2 style="color: ${isOverdue ? "#ef4444" : "#4a90e2"
        }; text-align: center;">
          ...
          <div style="background-color: #f9fafb; padding: 20px; border-radius: 10px; margin: 20px 0; border-left: 4px solid ${isOverdue ? "#ef4444" : "#4a90e2"
        };">
            ...
            <p style="margin: 5px 0; color: #333;"><strong>Your Share:</strong> <span style="font-size: 24px; color: ${isOverdue ? "#ef4444" : "#4a90e2"
        }; font-weight: bold;">$${amount}</span></p>
            <p style="margin: 5px 0; color: #333;"><strong>${isOverdue ? "Was Due" : "Due Date"
          ...
          <div style="text-align: center; margin: 30px 0;">
            <a href="${paymentLink}" style="background-color: ${isOverdue ? "#ef4444" : "#4a90e2"
      ...
      text: `
        ...
        Please visit ${paymentLink} to ${isOverdue ? "pay now" : "view payment details"
        }.
