// ================================================================
// src/hooks/useSubscriptions.ts - UPDATED WITH REDUX INTEGRATION
// ================================================================
...
import { playSound } from "@/features/auth/notificationSoundSlice";
...
export const useSubscriptions = () => {
  ...
  return useInfiniteQuery({
    ...
    queryFn: async ({ pageParam = 1 }) => {
      if (!user?._id)
        return { data: [], pagination: { totalPages: 0, currentPage: 1 } };
      const response = await api.get(
        `/subscriptions/user/${user._id}?page=${pageParam}&limit=10`
      );
